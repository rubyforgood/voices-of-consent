...
class InventoryAdjustmentsController < ApplicationController
  before_action :set_inventory_adjustment, only: %i[show edit update destroy]
  ...
  def destroy
    ...
    respond_to do |format|
      format.html do
        redirect_to inventory_adjustments_url,
                    notice: 'Inventory adjustment was successfully destroyed.'
      end
