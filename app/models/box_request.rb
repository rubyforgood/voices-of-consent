...
class BoxRequest < ApplicationRecord
  ...
  belongs_to :reviewed_by,
             ...
             inverse_of: :box_requests_as_reviewer
  ...
  def create_associated_box
    Box.create(box_request: self) unless box

    box.create_core_box_items! if box.box_items.none?
  end
  ...
  def decline_review!
    review_declined_by_ids << current_user.id
    save!
  end
