...
RSpec.describe InventoryImportsController, type: :controller do
  ...
  let(:invalid_params) do
    { import_file: fixture_file_upload('files/inventory_tally_malformed.csv', 'text/csv') }
  end
  ...
  describe 'POST #create' do
    context 'with valid params' do
      it 'creates new InventoryTally objects' do
        expect { post :create, params: valid_params, session: valid_session }.to change(
          InventoryTally,
          :count
        ).by(4)
      end
    ...
    context 'with invalid params' do
      ...
      it 'does not create inventory tallies' do
        expect { post :create, params: invalid_params, session: valid_session }.not_to change(
          InventoryTally,
          :count
        )
      end
